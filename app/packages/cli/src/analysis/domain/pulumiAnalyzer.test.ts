...
import { PulumiAnalyzer } from './pulumiAnalyzer.ts';
...
`
  ...
  it('links multiple pulumi projects under one Infrastructure hub', async () => {
    const fs = new MockFileSystem();
    const scan = path.resolve('/repo');
    ...
    fs.directories.set(a, ['Pulumi.yaml']);
    fs.directories.set(b, ['Pulumi.yaml']);
    fs.textFiles.set(
      path.resolve('/repo/stack-a/Pulumi.yaml'),
      `name: a\nruntime: yaml\nresources:\n  bucket:\n    type: aws:s3:Bucket\n`
    );
    fs.textFiles.set(
      path.resolve('/repo/stack-b/Pulumi.yaml'),
      `name: b\nruntime: yaml\nresources:\n  queue:\n    type: aws:sqs:Queue\n`
    );
    fs.existingFiles.add(path.resolve('/repo/stack-a/Pulumi.yaml'));
    fs.existingFiles.add(path.resolve('/repo/stack-b/Pulumi.yaml'));
