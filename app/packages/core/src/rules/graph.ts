...
/** Zod contract for blueprint YAML/JSON. Prefer this over hand-written JSON Schema. */
 ...
 * Documents are a one-element YAML sequence (`- entityRef: …`).
...
export function toSystemSchemaJsonSchema(): Record<string, unknown> {
  const itemSchema = z.toJSONSchema(systemSchemaValidator, { target: 'draft-07' }) as Record<
    string,
    unknown
  >;
  ...
  return {
    ...
    description:
      ...
      'Each file is a one-element YAML sequence whose first key is entityRef when present. ' +
...
/** Accept legacy object-root docs or the current one-element sequence form. */
...
export function parseSchemaFromYaml(yamlContent: string): SystemSchema {
  ...
  try {
    return mapValidatedSchema(systemSchemaValidator.parse(unwrapSchemaDocument(parsed, 'YAML')));
  } catch (zodErr) {
...
export function parseSchemaFromJson(jsonContent: string): SystemSchema {
  ...
  try {
    return mapValidatedSchema(systemSchemaValidator.parse(unwrapSchemaDocument(parsed, 'JSON')));
  } catch (zodErr) {
    if (zodErr instanceof z.ZodError) {
 ...
 * Root is a one-element sequence so the document opens with `- entityRef:` (when set).
 */
export function serializeSchemaToYaml(
  ...
  cleanSchema.name = schema.name;
  cleanSchema.version = schema.version;
  cleanSchema.level = schema.level;
  ...
  return `${blueprintYamlLanguageServerDirective(options?.schemaUrl)}\n${body}`;
}
