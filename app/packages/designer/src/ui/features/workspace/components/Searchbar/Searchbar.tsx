...
export const Searchbar: React.FC<SearchbarProps> = ({ collapsibleOnMobile = false }) => {
  ...
  useEffect(() => {
    if (!collapsibleOnMobile || !mobileExpanded || searchQuery) return;
  ...
  }, [collapsibleOnMobile, mobileExpanded, searchQuery, containerRef]);

  const collapseMobileIfEmpty = () => {
    if (collapsibleOnMobile && !searchQuery) {
      setMobileExpanded(false);
    }
  };
  ...
  return (
    ...
    >
      {collapsibleOnMobile && !mobileExpanded ? (
        <button
          type="button"
          className="sm:hidden min-h-11 min-w-11 flex items-center justify-center rounded-lg bg-slate-900/40 border border-slate-850 text-slate-400 hover:text-slate-200 hover:border-slate-800 transition cursor-pointer"
          onClick={() => {
      ...
      <div
        className={collapsibleOnMobile ? (mobileExpanded ? 'flex' : 'hidden sm:block') : 'block'}
      >
        <div className="bg-slate-900/40 border border-slate-850 hover:border-slate-800 focus-within:border-brand-500 rounded-lg transition-colors flex items-center gap-2 px-2.5 py-1.5">
          ...
          <input
            ...
            onKeyDown={e => {
              if (e.key === 'Escape' && collapsibleOnMobile && !searchQuery) {
                setMobileExpanded(false);
              }
              handleKeyDown(e);
            }}
                ...
                {filteredNodes.map((node, index) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      onClick={() => {
                        handleSelectNode(node.entityRef || '');
                        collapseMobileIfEmpty();
                      }}
